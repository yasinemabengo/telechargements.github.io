...
function displayInscriptions(data) {
        ...
        tbody.append(`
            <tr>
                <td colspan="8" class="text-center text-muted">
                    <i class="bi bi-inbox"></i> Aucune inscription enregistrée
                </td>
            </tr>
        `);
        return;
    ...
    data.forEach(function(insc) {
        const fullName = `${insc.prenom || ''} ${insc.nom || ''} ${insc.postnom || ''}`;
        ...
        if (insc.statut_inscription === 'En cours') statusBadge = 'bg-info';
        
        const isTransfered = insc.transfere_etudiant == 1;
        const transferBtn = isTransfered ? 
            '<button class="btn btn-secondary" disabled title="Déjà ajouté comme étudiant"><i class="bi bi-check-circle"></i></button>' :
            `<button class="btn btn-success" onclick="transferToEtudiant(${insc.id_inscription})" title="Enregistrer comme étudiant"><i class="bi bi-person-plus"></i></button>`;
            ...
            <tr>
                <td>
                    <div class="btn-group btn-group-sm" role="group">
                        <button class="btn btn-info" onclick="viewInscription(${insc.id_inscription})" title="Voir plus">
                            <i class="bi bi-eye"></i>
                        </button>
                        <button class="btn btn-warning" onclick="editInscription(${insc.id_inscription})" title="Modifier">
                            <i class="bi bi-pencil"></i>
                        </button>
                        ${transferBtn}
                        <button class="btn btn-danger" onclick="deleteInscription(${insc.id_inscription})" title="Supprimer">
                ...
                <td>${insc.nom_faculte || 'N/A'}</td>
                <td>${insc.nom_filiere || 'N/A'}</td>
        ...
        success: function(response) {
            if (response.success && response.data) {
                ...
                const statusColor = insc.statut_inscription === 'Validée' ? 'success' : 
                                   insc.statut_inscription === 'En cours' ? 'info' : 
                                   insc.statut_inscription === 'Rejetée' || insc.statut_inscription === 'Annulée' ? 'danger' : 'warning';
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-2"><strong>Sexe:</strong> ${insc.sexe}</div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6 mb-2"><strong>Premier Choix:</strong> ${insc.premier_choix}</div>
                                        <div class="col-md-6 mb-2"><strong>Deuxième Choix:</strong> ${insc.deuxieme_choix || 'N/A'}</div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-4 mb-2"><strong>Faculté:</strong> ${insc.nom_faculte || 'N/A'}</div>
                                        <div class="col-md-4 mb-2"><strong>Filière:</strong> ${insc.nom_filiere || 'N/A'}</div>
...
function editInscription(id) {
    window.location.href = `../views/ajout_inscription.php?edit=${id}`;
}
...
function editInscription(id) {
    // Rediriger vers ajout_inscription.php avec l'ID
    window.location.href = 'ajout_inscription.php?edit=' + id;
}
...
function transferToEtudiant(id) {
    if (confirm('Voulez-vous transférer cette inscription vers le formulaire étudiant ?')) {
        ...
        window.location.href = '../views/ajout_etudiant.php?from_inscription=' + id;
    }
}
