            ...
            <tr>
                <td style="position: sticky; left: 0; background-color: #fff; z-index: 1;">
                    <div class="btn-group btn-group-sm" role="group">
                        ...
                        <button class="btn btn-warning" onclick="editEnseignant(${ens.id_enseignant})" title="Modifier">
...
$('#formAddEnseignant').on('submit', function(e) {
    e.preventDefault();
    
    const formData = new FormData(this);
    const action = $('#id_enseignant').val() ? 'update' : 'create';
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                alert(action === 'create' ? 'Enseignant ajouté avec succès' : 'Enseignant modifié avec succès');
                resetForm();
                loadEnseignants();
            } else {
    ...
    $.ajax({
        ...
        success: function(ens) {
            if (ens) {
                ...
                $('#id_enseignant').val(ens.id_enseignant);
                $('#matricule_enseignant').val(ens.matricule_enseignant);
                $('#nom').val(ens.nom);
                $('#postnom').val(ens.postnom);
                $('#prenom').val(ens.prenom);
                $('#sexe').val(ens.sexe);
                $('#date_naissance').val(ens.date_naissance);
                $('#lieu_naissance').val(ens.lieu_naissance);
                $('#nationalite').val(ens.nationalite);
                $('#grade').val(ens.grade);
                $('#specialite').val(ens.specialite);
                $('#diplome_obtenu').val(ens.diplome_obtenu);
                $('#annee_obtention_diplome').val(ens.annee_obtention_diplome);
                $('#universite_origine').val(ens.universite_origine);
                $('#id_faculte').val(ens.id_faculte);
                $('#departement').val(ens.departement);
                $('#statut_emploi').val(ens.statut_emploi);
                $('#adresse').val(ens.adresse);
                $('#telephone').val(ens.telephone);
                $('#email').val(ens.email);
                $('#date_engagement').val(ens.date_engagement);
                $('#numero_ordre').val(ens.numero_ordre);
                $('#statut').val(ens.statut);
                $('#numero_compte').val(ens.numero_compte);
                $('#banque').val(ens.banque);
                ...
                $('html, body').animate({
                    scrollTop: $('#formAddEnseignant').offset().top - 100
                }, 500);
            }
        },
        error: function() {
                ...
                modalBody.html(`
                    <div class="row">
                        ...
                        <div class="col-md-3 text-center mb-4">
                            <div class="card">
                                <div class="card-body">
                                    ${ens.photo ? `<img src="../uploads/${ens.photo}" class="img-fluid rounded mb-3" style="max-height: 250px; object-fit: cover;" alt="Photo">` : '<div class="bg-secondary text-white rounded d-flex align-items-center justify-content-center mb-3" style="height: 250px; font-size: 80px;"><i class="bi bi-person-circle"></i></div>'}
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="card mb-3">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="mb-2">
                                                <strong><i class="bi bi-file-pdf"></i> CV:</strong> ${ens.cv ? `<a href="../uploads/${ens.cv}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-download"></i> Télécharger CV</a>` : 'Non disponible'}
                                            </div>
                                            <div class="mb-2">
                                                <strong><i class="bi bi-folder"></i> Dossier:</strong> ${ens.dossier ? `<a href="../uploads/${ens.dossier}" target="_blank" class="btn btn-sm btn-outline-success"><i class="bi bi-download"></i> Télécharger Dossier</a>` : 'Non disponible'}
                                            </div>
                                        </div>
                                    </div>
